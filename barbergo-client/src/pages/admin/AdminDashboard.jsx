...
import { LayoutDashboard, Scissors, Users, Plus, Edit, Trash2, Save, X } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

export default function AdminDashboard() {
  const { user, logout } = useAuth();
  const [activeTab, setActiveTab] = useState('shop'); // shop, services, barbers
  ...
  const fetchData = async () => {
    ...
    try {
      const [shopRes, servicesRes, barbersRes] = await Promise.all([
        api.get('/admin/barbershop').catch(() => ({ data: null })), // Handle 404 if no shop yet? But middleware ensures admin has shop access logic usually.
  ...
  const updateShop = async (e) => {
    e.preventDefault();
    try {
        const { data } = await api.put('/admin/barbershop', shopForm);
        setShop(data);
  ...
  const saveService = async (e) => {
    ...
    try {
        if (editId) {
            await api.put(`/admin/layanan/${editId}`, serviceForm);
        } else {
            await api.post('/admin/layanan', serviceForm);
        }
  ...
  const saveBarber = async (e) => {
    ...
    try {
        if (editId) {
             await api.put(`/admin/tukang-cukur/${editId}`, barberForm);
        } else {
             await api.post('/admin/tukang-cukur', barberForm);
        }
  ...
  const deleteBarber = async (id) => {
    ...
    try {
        await api.delete(`/admin/tukang-cukur/${id}`);
